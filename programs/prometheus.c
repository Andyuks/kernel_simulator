...
int main(int argc, char *argv[]) {
    ...
    for(pnum = conf.first_number; pnum < (conf.first_number + conf.how_many); pnum++){
        ...
        code_size = 8 + (rand() % conf.max_lines); // Gutxienez, agindu multzo bat 
        data_start = (code_start + ((code_size >> 3) << 3) + 1) << 2;
        data_size = 8 + (rand() % conf.max_lines);
        ...
        for (i=0; i < (code_size >> 3); i++) { //  8 lerrotako blokeak: ld ld add st wrdisk wrdisk lddisk wrdisk
            reg1 = rand() % 16;
            ...
            sprintf(line,"4%1X%1X00000\n", reg2, reg1); // write value reg2 in page reg1
            fputs(line, fd);
            sprintf(line,"4%1X%1X00000\n", reg3, reg2); // write value reg3 in page  (reg1+1)%16)
            fputs(line, fd);
            sprintf(line,"3%1X%1X00000\n", (reg3+1)%16, reg1); // read value stored in page reg1 and leave it in register (reg3+1)%16)
            fputs(line, fd);
            sprintf(line,"4%1X%1X00000\n", reg1, reg3); // write value reg1 in page reg3
            fputs(line, fd);
